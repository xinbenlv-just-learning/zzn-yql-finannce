...
var queryGetAllTickers = 'select * from yahoo.finance.industry where id in (select industry.id from yahoo.finance.sectors) and id >=800 and id<900'
new YQL(queryGetAllTickers).exec(function (error, response) {
    ...
    for (var i in industries) {
        ...
        if (parseInt(industry.id)<800 || parseInt(industry.id) >899) continue;
